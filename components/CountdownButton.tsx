...
const CountdownButton = ({glitch, scanLines, label, className}: { glitch?: boolean, scanLines?: boolean, label?: string, className?: string }) => {
    ...
    return (
        <motion.button
            ...
            className={clsx("group relative overflow-hidden bg-[#181818] disabled:opacity-50 duration-200 active:scale-95 hover:bg-white hover:text-black transition-all border-b px-7 py-4 my-5 text-white text-xl flex items-center gap-2 work-sans w-[80%] max-w-[25rem] md:w-[25rem] justify-center", className)}
        >
            <div className="relative z-10 flex items-center gap-2">
                <span>{isPending ?
                    <div className="items-center flex gap-2"><FiLoader className="animate-spin"/> CONQUEST BEGINS
                    </div> : glitch ? textHover ? text : 'BEGIN CONQUEST' : label ? label : 'BEGIN CONQUEST'}
                </span>
