...
/*    let mouseX = useMotionValue(0);
    ...
    return (
     ...
     {/*           <div className='h-fit my-auto'>
            ...
            <div className='overflow-hidden py-20 flex flex-col gap-40 xl:gap-24 mt-4'>
                {cards.map(card => (
                    <div key={card.id} className={clsx('flex md:w-[95%] lg:w-5/6 xl:w-full mx-auto gap-2 relative flex-col xl:flex-row justify-between', card.id % 2 === 0 ? 'xl:flex-row-reverse px-8 lg:px-12' : 'px-8 lg:px-12')}>
                        <motion.div transition={{type: 'spring', damping: 20}} initial={card.reverse ? {x: 100, opacity: 0} : {x: -150, opacity: 0}} whileInView={{x: 0, opacity: 1}} viewport={{once: true}}
                                    className='z-10 shadow-[0_0px_30px_rgba(5,_5,_5,_0.3)] shadow-black rounded-xl w-full h-[50svw] xl:w-[600px] xl:h-[400px] relative'>
