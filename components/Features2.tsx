    ...
    return (
        <div className="flex overflow-x-hidden flex-col lg:flex-row w-full relative lg:w-fit mx-auto pb-24">
            ...
            <motion.div className="px-6 lg:items-start lg:text-start items-center text-center flex flex-col" viewport={{once: true}} initial={{opacity: 0}} transition={{type: 'tween', duration: 1}} whileInView={{opacity: 1}}>
                ...
                <h1 className=" text-6xl lg:text-6xl xl:text-8xl font-medium lg:mb-0 mb-4">
                    <mark className="bg-transparent text-white font-semibold">Innovative </mark> <br className="hidden lg:flex"/> Features
                </h1>
                <div className='p-5 flex flex-col gap-8 lg:gap-10'>
                    {features.map((feature, index) => (
                        <motion.div
                            ...
                            <h1 className='text-xl text-start'>{feature.text}</h1>
                        </motion.div>
                    ))}
                </div>
            </motion.div>
